...
void Vector2dTest(double precision)
{
    ...
    //Miscellaneous operator test
    {
        ...
        result = dot(u,v);
        if(result - 41.471 > precision){Error("Dot product test");}
        result = sqlength(u);
        if(result - 186.81216 > precision){Error("Square length test");}
        result = length(u);
        if(result - 13.66792449496265 > precision){Error("Length test");}
        v = normalize(u);
        if(length(v) - 1.0 > precision){Error("Normalize test");}
    }
